...
function extractGalleryZip($zipPath, $uploadDir, $compress, $maxWidth = 1600) {
    ...
    for ($i = 0; $i < $zip->numFiles; $i++) {
        ...
        if (!in_array($ext, $allowedExt) || strpos($entry, '__MACOSX') !== false || strpos(basename($entry), '._') === 0) continue;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verifyCsrf()) {
    ...
    if ($category === 'videos') {
        ...
        if ($title && $youtubeUrl) {
            ...
            $ytId = $m[1] ?? '';
            if ($ytId) {
                $stmt = $db->prepare("INSERT INTO gallery_items (title, category, description, event_name, event_date, tags, visibility, is_featured, file_path, file_type, uploaded_by, approved_by, approved_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'video', ?, ?, NOW(), 'approved')");
                $stmt->execute([$title, $category, $description, $eventName ?: null, $eventDate, $tags ?: null, $visibility, $isFeatured, $youtubeUrl, $uid, $uid]);
                auditLog('admin_upload_video', 'gallery_item', (int)$db->lastInsertId(), "Video: $title");
                setFlash('success', 'Video uploaded and auto-approved.');
            } else {
        ...
        header('Location: /admin/upload-gallery.php');
        exit;
    }
        ...
        if ($zipFile['size'] > $maxZip) {
            ...
            if ($results === false || empty($results)) {
                ...
                foreach ($results as $idx => $r) {
                    ...
                    $stmt->execute([$imgTitle, $category, $description, $eventName ?: null, $eventDate, $tags ?: null, $visibility, $isFeatured, $r['path'], $r['original_size'], $r['compressed_size'], $batchId, $uid, $uid]);
                    $successCount++;
                }
                auditLog('admin_upload_zip', 'gallery_item', 0, "ZIP batch $batchId: $successCount images");
                setFlash('success', "$successCount image(s) extracted from ZIP, uploaded and auto-approved.");
            }
        }
        header('Location: /admin/upload-gallery.php');
        exit;
    ...
    if (!empty($_FILES[$fileKey]['name'])) {
        ...
        foreach ($files as $idx => $file) {
            ...
            $stmt->execute([$imgTitle, $category, $description, $eventName ?: null, $eventDate, $tags ?: null, $visibility, $isFeatured, $finalPath, $origSize, $compSize, $useBatch, $uid, $uid]);
            $successCount++;
        }

        if ($successCount > 0) {
            auditLog('admin_upload', 'gallery_item', 0, "Batch: $successCount images");
            setFlash('success', "$successCount image(s) uploaded and auto-approved." . ($failCount ? " $failCount skipped." : ''));
        } else {
...
$items = $db->prepare("SELECT id, title, category, file_path, file_type, status, batch_id, original_size, compressed_size, is_featured, created_at FROM gallery_items WHERE uploaded_by=? ORDER BY created_at DESC LIMIT ? OFFSET ?");
                ...
                <form method="POST" enctype="multipart/form-data" id="uploadForm">
                    ...
                    <div class="form-check mb-3">
                        ...
                        <label class="form-check-label" for="featuredCheck">
                            <i class="bi bi-star text-warning"></i> Mark as Featured
                        </label>
                    </div>
                    <button type="submit" class="btn btn-primary w-100"><i class="bi bi-upload me-1"></i>Upload & Auto-Approve</button>
                </form>
            </div>
        </div>
    </div>
    <div class="col-lg-7">
        <div class="card border-0 shadow-sm">
                    ...
                    <div class="col-sm-6 col-md-4">
                        <div class="card h-100 border-0 shadow-sm position-relative">
                            ...
                            <div class="card-body p-2">
                                ...
                                <?php if ($item['is_featured']): ?><span class="badge bg-warning mt-1"><i class="bi bi-star-fill"></i> Featured</span><?php endif; ?>
            ...
            <?php if ($p['total_pages'] > 1): ?>
                <div class="card-footer bg-white"><?= paginationHtml($p, '/admin/upload-gallery.php') ?></div>
            <?php endif; ?>
        </div>
    </div>
</div>
...
function deleteUpload(btn) {
    ...
    if (confirm('Are you sure you want to delete this upload? This action cannot be undone.')) {
        ...
        formData.append('csrf_token', document.querySelector('input[name="_token"]')?.value || document.querySelector('input[name="csrf_token"]')?.value || '');
